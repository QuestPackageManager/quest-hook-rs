use quest_hook::hook;
use quest_hook::libil2cpp::{Il2CppObject, Il2CppType, Type};
...
// `MyMethod` only takes one parameter besides `this` - this hook declares an
// extra one that isn't there, so it should fail to compile.
#[hook(MyClass::MyMethod)]
fn my_hook(this: &mut Il2CppObject, value: &mut Il2CppObject, extra: &mut Il2CppObject) -> bool {
    my_hook.original(this, value, extra)
}
